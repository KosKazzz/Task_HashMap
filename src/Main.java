...
public class Main {
    ...
    public static String text = "Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et " +
    ...
    public static void main(String[] args) {
        ...
        int max = -1;
        char mostPop = '`';
        for (Map.Entry<Character, Integer> entry : charsInText.entrySet()) {
            if (entry.getValue() > max) {
                max = entry.getValue();
                mostPop = entry.getKey();
            }
        }
        System.out.println(" Чаще всех встречается символ: \"" + mostPop + "\", повторяется : " + max + " раз(a)!");
        ...
        System.out.println(" Реже всех встречается символ: \"" + mostPop + "\" , повторяется : " + min + " раз(a)!");
        //test
        for (Map.Entry<Character, Integer> entry : charsInText.entrySet()) {
            char c = entry.getKey();
